...
class VideoProcessor(QThread):
    ...
    def __init__(self, video_path, output_dir, target_width, split_parts, max_size_mb, filename_prefix, rename_strategy, method, hex_value=None, fps=15, full_gif_name="output.gif"):
        super().__init__()
...
#作案变量
    ...
    def add_ffmpeg_to_path():
        if getattr(sys,'frozen', False):
            ...
            base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "ffmpeg"))
    ...
    def convert_with_ffmpeg(self, output_path):
        ...
        ffmpeg_cmd = [
            ...
            "-vf", f"fps={self.fps},scale={self.target_width}:-1:flags=lanczos",
            "-c:v", "gif", output_path
        ]
    ...
    def split_gif(self, input_gif):
        ...
        for i in range(self.split_parts):
            ...
            frames = [frame.crop((left, 0, right, height)) for frame in ImageSequence.Iterator(gif)]
            part_output = os.path.join(self.output_dir, f"{self.filename_prefix}_{i+1}.gif")

            frames[0].save(part_output, save_all=True, append_images=frames[1:], loop=0, duration=int(1000 / self.fps), disposal=2)
    ...
    def compress_gif(self, input_gif):
        ...
        while os.path.getsize(input_gif) > self.max_size_mb * 1024 * 1024:
            ...
            command = [
                self.ffmpeg_path, "-y", "-i", input_gif,
                "-vf", "fps=10,scale=iw/2:ih/2:flags=lanczos",  #降低帧率&缩小尺寸
                "-b:v", "500k", "-gifflags", "+transdiff", "-y", temp_output
            ]
            subprocess.run(command, check=True)
            ...
            if os.path.getsize(input_gif) <= self.max_size_mb * 1024 * 1024:
                print(f"GIF {input_gif} 已压缩到 {os.path.getsize(input_gif) / 1024 / 1024:.2f}MB")
                break
